...
class TestLabyrinthGeneration(unittest.TestCase):
    ...
    def test_cannot_create_maze_with_large_amount_of_steps(self):
        ...
        self.assertRaises(UserInputError, maze.generate_maze)
...
class TestLabyrinthFeatures(unittest.TestCase):
        ...
        while len(stack) > 0:
            ...
            if self.maze.labyrinth_matrix[y][x] == "." and dfs_matrix[y][x] == "#":
                ...
                for direction in directions:
                    try:
                        if (
                            self.maze.labyrinth_matrix[y + direction[0]][
                                x + direction[1]
                            ]
                            == "."
                        ):
    ...
    def test_goal_is_reached_in_required_amount_of_steps(self):
        ...
        while len(stack) > 0:
            ...
            for direction in directions:
                try:
                    if self.maze.labyrinth_matrix[y + direction[0]][
                        x + direction[1]
                    ] == "." and bfs_matrix[y + direction[0]][
                        x + direction[1]
                    ] == float(
                        "inf"
                    ):
                        stack.append(
                            ((y + direction[0], x + direction[1]), distance + 1)
                        )
                except IndexError:
                    pass
